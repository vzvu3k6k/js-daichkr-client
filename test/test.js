...
const loggedInClient = new DaichkrClient({
  jar: new ToughCookieFileStore(
    path.resolve(__dirname, "./secrets/logined.jar")
  )
});
...
describe("Antenna", () => {
  ...
  describe("#fetchInfo", () => {
    it("should parse a public antenna", () => {
      ...
      return antenna.fetchInfo().then(info =>
  ...
  describe("#fetchEditInfo", () => {
    it("should parse an edit page of my antenna", () =>
      tempAntenna.fetchEditInfo().then(info =>
        assert.deepEqual(info, {
    ...
    it("should parse an edit page of an antenna which is not mine", () => {
      ...
      return antenna.fetchEditInfo().then(info =>
        assert.deepEqual(info, {
          title: "どんどんチェック (ひっそり)",
          name: "どんどんチェック",
          description: "js-daichkr-clientのテスト用",
          ...
          isMine: false,
          note:
            "大チェッカーで、しゅっとインターネットをチェックしてみませんか。"
        })
      );
    });
  });
